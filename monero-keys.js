 ...
 *   - Private spend key (seed â†’ sc_reduce32)
 *   - Private view key  (Keccak-256 of spend key â†’ sc_reduce32)
 *   - Public spend key  (ed25519 basepoint Ã— private spend key)
 *   - Public view key   (ed25519 basepoint Ã— private view key)
...
const MoneroKeys = (function () {
   ...
   * This is the core derivation â€” independent of mnemonic format.
   *
  ...
  function deriveFromMnemonic(mnemonic, lang, network) {
    ...
    if (count === 25) {
      // Standard Monero: 24 data + 1 checksum â†’ 32-byte seed
      if (!MoneroWordList.verifyChecksum(lang, words)) {
        throw new Error(
          'Invalid mnemonic â€” checksum did not verify against any of the ' +
      ...
      // MyMonero: 12 data + 1 checksum â†’ 16-byte partial â†’ Keccak â†’ 32 bytes
      if (!MoneroWordList.verifyChecksum(lang, words)) {
        throw new Error(
          'Invalid mnemonic â€” checksum did not verify against any of the ' +
          '13 supported wordlists. Double-check that you copied every word ' +
      ...
      throw new Error(
        ...
        `derivation â€” call MoneroKeys.deriveFromAnyMnemonic() instead.`
      );
    }
  ...
  function deriveFromSpendKey(spendKeyHex, network, lang) {
    ...
    // polyseed seeds â€” those go through one-way KDFs.)
    try {
    ...
    } catch (e) { /* wordlist missing â€” leave result.mnemonic undefined */ }
    return result;
  }
  ...
  async function deriveFromBip39(mnemonic, passphrase, network) {
    if (typeof Bip39 === 'undefined') {
      throw new Error('Bip39 module not loaded â€” include js/bip39.js first');
    }
  ...
  async function deriveFromPolyseed(mnemonic, network) {
    if (typeof Polyseed === 'undefined') {
      throw new Error('Polyseed module not loaded â€” include js/polyseed.js first');
    }
   ...
   * the word count.
   *   12 â†’ BIP-39  (uses Bip39 module + SubtleCrypto)
   *   13 â†’ MyMonero legacy (sync, wrapped in a Promise)
   *   16 â†’ Polyseed (uses Polyseed module + SubtleCrypto)
   *   25 â†’ Standard Monero (sync, wrapped in a Promise)
   */
