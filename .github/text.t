...
/**
 * lws-client.js â€” Browser client for monero-lws (light-wallet server)
 *
 ...
 * Trust model
 * -----------
 *   â€¢ What the server sees:  view key, primary address, signed tx hex
 *   â€¢ What the server NEVER sees:  spend key, seed phrase, mnemonic
 *
 * The view key alone cannot spend funds â€” it can only see incoming outputs.
 * The spend key stays in the user's browser tab forever, encrypted by the
 * existing WalletVault session password.
 *
 * Public API:
 *   await LwsClient.login(address, viewKey, opts)        â†’ register a wallet
 *   await LwsClient.getAddressInfo(address, viewKey)     â†’ balance + state
 *   await LwsClient.getAddressTxs(address, viewKey)      â†’ transaction history
 *   await LwsClient.getUnspentOuts(address, viewKey,...) â†’ outputs to spend
 *   await LwsClient.getRandomOuts(amounts, count)        â†’ ring decoys
 *   await LwsClient.submitRawTx(txHex)                   â†’ broadcast
 *   LwsClient.setBaseUrl(url)                            â†’ override default
 *   LwsClient.setMockMode(boolean)                       â†’ for tests / dev
 *
...
const LwsClient = (function () {
  ...
  // Default base URL. The browser talks directly to nginx on the VPS, which
  // maps the /lws/ prefix to the local monero-lws-daemon (127.0.0.1:8443).
  // (Was the Cloudflare Worker proxy; removed when the site left Cloudflare.)
  ...
  // â”€â”€ Session handling (invisible self-hosted proof-of-work) â”€â”€â”€â”€â”€â”€â”€â”€
  // Replaces Turnstile/mCaptcha. On demand we GET a signed challenge from
  // node.monero-web.com/lws/session, solve it in a Web Worker (js/pow-worker.js:
  // find a nonce where sha256("<c>.<nonce>") has d leading zero bits), then POST
  // the nonce back to receive an HMAC session token (30m) that is attached to
  // LWS requests via the X-Session-Token guards below. No widget, no click.
  var _sessionToken  = '';
  var _sessionPromise = null;

  // Solve the PoW off the main thread so the UI never freezes.
  ...
  function ensureSession () {
    if (MOCK || _sessionToken) return Promise.resolve();
    if (_sessionPromise) return _sessionPromise;
    _sessionPromise = (async function () {
      try {
        var ch = await (await fetch(BASE_URL + '/session', { method: 'GET' })).json();
        var nonce = await solvePow(ch.c, ch.d);
        var resp = await fetch(BASE_URL + '/session', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ c: ch.c, exp: ch.exp, d: ch.d, sig: ch.sig, nonce: nonce }),
        });
        var st = resp.headers.get('X-Session-Token');
        if (st) _sessionToken = st;
      } catch (e) {
        console.warn('[pow] session init failed:', e && e.message);
      } finally {
  ...
  // Start the challengeâ†’solveâ†’session handshake on load so the PoW overlaps
  // page load instead of blocking the first wallet request.
  function prewarm () {
    if (typeof document === 'undefined') return;
    try { ensureSession(); } catch (e) {}
  }
  if (typeof document !== 'undefined') {
    if (document.readyState !== 'loading') prewarm();
    else document.addEventListener('DOMContentLoaded', prewarm);
  }

  // â”€â”€ Internal POST helper â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  async function post (path, body) {
    ...
    for (;;) {
      ...
      try {
        ...
        throw new LwsError('network', 'Not enough XMR is available to process this transaction.: ' + e.message, e);
      }
    ...
    // Refresh session token if a new one is issued
    var st = response.headers.get('X-Session-Token');
    if (st) _sessionToken = st;
    let data;
    try { data = await response.json(); }
    catch (e) {
      // Empty or non-JSON response â€” check HTTP status first
      if (!response.ok) {
        throw new LwsError(
          'server',
  ...
  // â”€â”€ Public endpoints â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

  /**
   * Register a wallet's view key with the server. Idempotent â€” calling
  ...
  async function login (address, viewKey, opts) {
    ...
    // this field in /login â€” it always registers at the current chain tip.
  ...
  // â”€â”€ Helpers exposed to callers â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  function scanProgress (info) {
    ...
    if (cur >= tip)   return 1;
    // Treat "within 3 blocks of chain tip" as fully synced â€” the chain
  ...
  // â”€â”€ Custom error type â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â”€â”€ Mock backend (used for UI development without a running LWS) â”€â”€
  ...
  function handleMock (path, body) {
    ...
    // UI shows a "scanningâ€¦" state for the first ~20 seconds.
   ...
   * Fire-and-forget â€” failures are silently ignored so the wallet
  ...
  async function pingLogin (address) {
    ...
    try {
      ...
      // Non-critical â€” don't break the login flow
